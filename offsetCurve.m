...
function [joinedx, joinedy] = offsetCurve(x, y, offset, haxes)
% Offset a curve by a given distance

intersectremove = 1;
if nargin < 5, intersectremove = 0; end
iPt=2;
eps = 10^-10;
...
if offset>0
    joinedx(end+1) =joinedx(1);
    joinedy(end+1) =joinedy(1);
else
    bod = InterX([joinedx;joinedy]);
    joinedx(1) = bod(1);
    joinedx(end) = bod(1);
    joinedy(1) = bod(2);
    joinedy(end) = bod(2);
